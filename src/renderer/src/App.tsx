  ...
  const guides: Array<{
    ...
    },
    {
      ...
      tips: [
        ...
        "0.1.37 以前的任务继续使用当时的扫描规则，不会在恢复时暗中扩大范围；新任务的范围与排除项会冻结在任务记录中。",
    ...
    },
    {
      ...
      steps: [
        ...
        "若只需恢复原项目缺失的目录，使用“只补齐原项目目录”；该操作按当前已保存规则执行，不会保存表单中尚未确认的新目的地或其他修改。",
    ...
    },
    {
      ...
      tips: [
        "PDF 可包含素材首帧缩略图。",
        "大型 PDF 从权限受限的临时页面生成，加载后立即清理；报告失败不会改变素材、校验记录或任务完成状态。",
  ...
  const releaseNotes = [
    {
      ...
      title: "界面边界与折叠更新记录",
      paragraphs: [
        "偏好设置的保存状态回到卡片标题区，不再贴住内容分隔线；新建备份的路径添加入口和素材卷卡片补齐明确的操作边界。",
        "普通备份与项目备份选择卡统一图标容器和选中标记；软件使用说明中的各版本更新改为独立折叠栏，默认保持收起。",
      ],
    },
    {
      version: "0.1.38",
    ...
    {
      version: "0.1.36",
      title: "项目规则确认与大型 PDF 报告",
      paragraphs: [
        "编辑项目时会先逐项显示安全规则的原值和新值，只有真实规则变化才要求填写修改人；原项目目录补齐可与保存表单新规则分开执行。",
        "单任务、拍摄日、项目和自动报告统一从权限受限的临时页面生成 PDF，加载后立即清理，避免大量缩略图导致超长地址错误。",
      ],
    },
    {
      version: "0.1.35",
      ...
      paragraphs: [
        ...
        "可选真实介质测试要求先查看计划并显式确认，只写入随机合成测试目录，拒绝重复文件系统或同一整盘被误算成多个目标，并输出包含清理状态的 JSON 证据。当前仍无 Developer ID 与 Apple 公证。",
      ],
    },
    {
      version: "0.1.34",
      title: "重复导入幂等与准确审计提示",
      paragraphs: [
        "内置模板的稳定时间值不再被导入过程改写；相同工作站包与相同冲突决定再次确认时直接返回原审计，不增加修订或追加审计。",
        "同一包若选择不同冲突决定仍可明确再次提交。按钮与提示会区分“核对既有审计”和“按当前决定提交”，避免把两种行为混为一谈。",
      ],
    },
    {
      version: "0.1.33",
    ...
    },
    {
      ...
      title: "稳定工作站身份与可审计合并",
      paragraphs: [
        "每台 Mac 使用跨重启稳定的随机身份。工作站包记录唯一导出 ID、权威工作区依据、交换摘要和删除墓碑；身份或恢复记录损坏时停止合并，不猜测重建。",
        "导入先显示只读预检；同 ID、内容、项目字段、追加证据和删除冲突默认保留本机。只有逐项选择外部值、填写操作人并确认后才提交，没有批量覆盖。",
    ...
    <div className="help-center">
      ...
      <section className="help-release-note">
        ...
        <div className="help-release-list">
          <strong>版本更新</strong>
          <small>按版本展开查看更新内容</small>
          {releaseNotes.map(({ version, title, paragraphs }) => (
            <details className="help-release-item" key={version}>
              <summary>
                <span>
                  <b>{version}</b>
                  <strong>{title}</strong>
      ...
      <section className="panel settings-panel">
        ...
        <div className="setting-row">
          <div>
            <h3>包含隐藏文件</h3>
            <p>
              默认完整纳入隐藏项、AppleDouble、.DS_Store 与空目录；关闭后会冻结并记录明确排除清单。
            </p>
          </div>
          <button
            role="switch"
        ...
        <div className="setting-row">
          <div>
            <h3>完成后自动生成 PDF</h3>
            <p>
              新任务默认在每个已校验备份目录内生成一份验收报告；仍可在新建备份时为单个任务关闭。
            </p>
          </div>
          <button
            role="switch"
            aria-label="默认自动生成 PDF"
            aria-checked={draft.automaticPdf}
            className={`switch ${draft.automaticPdf ? "on" : ""}`}
            onClick={() =>
              setDraft({ ...draft, automaticPdf: !draft.automaticPdf })
            }
          >
            <i />
          </button>
