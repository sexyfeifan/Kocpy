...
export function generateMhl(task: BackupTask) {
  ...
  const entries = task.fileRecords.map((file) => `  <hash><file>${xml(file.relativePath)}</file><size>${file.size}</size><${hashTag}>${file.srcChecksum}</${hashTag}></hash>`).join("\n");
  return `<?xml version="1.0" encoding="UTF-8"?>\n<mhl version="1.1">\n<creatorinfo><tool>Kocpy</tool><version>0.1.11</version></creatorinfo>\n<hashlist>\n${entries}\n</hashlist>\n</mhl>\n`;
}
