...
<div class="header">
  <div>
    <h1>Kocpy</h1>
    <p>VERIFIED MEDIA TRANSFER REPORT · v0.0.9</p>
...
export async function generateProjectReport(project: ProjectConfig, tasks: BackupTask[]): Promise<Buffer> {
  ...
  const matrixRows = dates.flatMap((shootingDate) => devices.map((device) => {
    ...
    const files = rows.reduce((sum, task) => sum + task.totalFiles, 0), size = rows.reduce((sum, task) => sum + task.totalBytes, 0), passed = rows.filter((task) => task.status === "completed").length;
    return `<tr><td>${esc(shootingDate)}</td><td>${esc(device)}</td><td>${rows.length}</td><td>${files}</td><td>${formatBytes(size)}</td><td class="${rows.length && passed === rows.length ? "ok" : rows.length ? "warn" : "muted"}">${rows.length ? `${passed} / ${rows.length} 已校验` : "尚未备份"}</td></tr>`;
  })).join("");
