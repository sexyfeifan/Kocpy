...
export function buildArchiveTransferDetailHtml(
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>${baseStyle}
    ...
    <header class="hero"><div><div class="eyebrow">Kocpy · ARCHIVE TRANSFER</div><h1>${escapeHtml(data.archiveName)}</h1><p class="subtitle">归档名来源：${escapeHtml(data.archiveNameSource)} · 拍摄日期：${escapeHtml(data.shootingDate)}</p></div><div class="result"><span class="dot"></span>逐文件回读${escapeHtml(data.verificationConclusion)}</div></header>
    ...
    <section class="section"><h2>实际路径</h2><div class="grid paths">${field("源项目文件夹", data.sourcePath, true)}${field("实际最终归档目标", data.finalPath, true)}</div></section>
...
export function buildArchiveTransferSummaryHtml(
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>${baseStyle}
    ...
    <header class="hero"><div><div class="eyebrow">Kocpy · ARCHIVE TRANSFER</div><h1>${escapeHtml(data.archiveName)}</h1><p class="subtitle">${escapeHtml(data.shootingDate)} · 名称来自${escapeHtml(data.archiveNameSource)}</p></div><div class="result"><span class="dot"></span>${escapeHtml(data.hashAlgorithm)} 独立回读${escapeHtml(data.verificationConclusion)}</div></header>
    ...
    <section class="paths">${field("源", data.sourcePath, true)}${field("实际最终归档目标", data.finalPath, true)}</section>
