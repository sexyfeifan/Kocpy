...
describe("archive lifecycle and workstation merge", () => {
  it("creates stable content fingerprints and detects duplicate card structures", () => {
    ...
    const suggestion = sourceSuggestion([task("a")], {
      files: [{ relativePath: "DCIM/clip.mov", size: 42 }],
    });
    expect(suggestion.duplicateTaskId).toBe("a");
    expect(suggestion.nextVolume).toBe(2);
  ...
  it("merges workstation records without duplicating identical media", () => {
    const merged = mergeWorkspace(
      { projects: [project], tasks: [task("a")] },
      {
        projects: [{ ...project, namingRule: "custom" }],
        tasks: [task("b"), task("c", "different")],
      },
    );
    expect(merged.tasks).toHaveLength(2);
    expect(merged.result.duplicates).toBe(1);
    expect(merged.result.tasksAdded).toBe(1);
    expect(merged.projects[0].namingRule).toBe("custom");
  });
  it("merges append-only project evidence without losing either workstation", () => {
    const local = {
      ...project,
      handoffNotes: [
        { id: "local-handoff", at: 1, operator: "A", note: "local" },
      ],
      dailyPlanDecisions: [
        {
          id: "local-plan",
    ...
    const remote = {
      ...
      namingRule: "remote",
      handoffNotes: [
        { id: "remote-handoff", at: 2, operator: "B", note: "remote" },
      ],
      dailyPlanDecisions: [
        {
          id: "remote-plan",
    ...
    const merged = mergeWorkspace(
      { projects: [local], tasks: [] },
      { projects: [remote], tasks: [] },
    );
    ...
    expect(
      merged.projects[0].dailyPlanDecisions?.map((item) => item.id),
    ).toEqual(["local-plan", "remote-plan"]);
  });
  it("turns project closeout settings into reusable templates", () => {
    const template = templateFromProject({
      ...project,
      completionActions: ["report", "eject"],
      volumePrefixByDevice: { A: "CAM_A_" },
      checklists: [
        { id: "close", phase: "close", label: "交接", required: true },
      ],
    });
    expect(template.requiredCopies).toBe(2);
    expect(template.completionActions).toEqual(["report", "eject"]);
  ...
  it("reports logical media volumes instead of inflating repeated attempts", () => {
    ...
    expect(
      projectCoverage({ ...project, requiredCopies: 1 }, [first, retry]),
    ).toMatchObject({
      recorded: 1,
      verified: 1,
      compliant: 1,
  ...
  it("accepts older valid records while rejecting malformed imports", () => {
    ...
    expect(() =>
      validateWorkspacePackage({
        ...old,
        tasks: [{ ...task("bad"), status: "owned" }],
      }),
    ).toThrow(/状态或哈希算法/);
    expect(() =>
      validateWorkspacePackage({ ...old, projects: "not-an-array" }),
    ).toThrow(/数据列表/);
    expect(() =>
      validateWorkspacePackage({ application: "Elsewhere", schema: 1 }),
    ).toThrow(/受支持/);
    expect(() => validateWorkspacePackage({ ...old, schema: 2 })).toThrow(
      /完整性摘要/,
    );
    expect(() =>
      validateWorkspacePackage({
