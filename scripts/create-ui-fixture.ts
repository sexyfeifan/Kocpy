...
async function main() {
  ...
  const data = path.join(root, "data"),
    source = path.join(root, "合成素材_不是正式素材");
  const target = path.join(root, "备份父目录"),
    other = path.join(root, "第二个备份父目录"),
    catalogCopy = path.join(root, "素材库合成副本");
  for (const dir of [data, source, target, other, catalogCopy])
    await fs.mkdir(dir);
  ...
  const catalogFiles = [];
  for (let i = 0; i < 300; i++) {
    const name = `合成_${String(i).padStart(4, "0")}.txt`,
      content = `Synthetic UI test ${i}\n`;
    await fs.writeFile(path.join(source, name), content, { flag: "wx" });
    if (i < 205) {
      const destination = path.join(catalogCopy, name),
      ...
      catalogFiles.push({
        ...
        destinations: [{ path: destination, checksum, verified: true }],
      });
    }
  }
  ...
  task.sourceVolumeId = sourceId.id;
  task.sourceVolumeUuid = sourceId.uuid;
  ...
  task.status = "failed";
  task.errorMessage =
    "磁盘 UUID 已变化，已停止操作（合成旧错误，用于原盘恢复验收）";
  task.projectId = "synthetic-project";
  const catalogTask = engine.createTask({
    name: "合成素材库分页验收",
    sourcePath: source,
    destinationPaths: [catalogCopy],
    devices: ["FX3"],
    shootingDate: "2026-09-01",
    namingTemplate: "CATALOG",
    copyMode: "normal",
    hashAlgorithm: "sha256",
    generateThumbnails: false,
    projectId: "synthetic-project",
  });
  Object.assign(catalogTask, {
    status: "completed",
    completedAt: Date.now(),
    totalFiles: catalogFiles.length,
    completedFiles: catalogFiles.length,
    totalBytes: catalogFiles.reduce((sum, item) => sum + item.size, 0),
    transferredBytes: catalogFiles.reduce((sum, item) => sum + item.size, 0),
    copyProgress: 100,
    verifyProgress: 100,
    fileRecords: catalogFiles,
  });
  for (const destination of catalogTask.destinations) {
  ...
  const project = {
    ...
    expectedDevicesByDate: { "2026-09-01": ["FX3"] },
    backupRoot: target,
  };
  await fs.writeFile(
  ...
  await fs.writeFile(
    ...
    JSON.stringify([task, catalogTask]),
  );
  console.log(
    JSON.stringify({
      root,
