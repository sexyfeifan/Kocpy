...
首页会根据任务状态显示当前传输、失败目标和待处理事项。项目可保存常用机位、目标位置及卷标前缀，连续接卡时自动递增 CARD001、CARD002 等卷号，并在开始前保留人工确认。

## 安全备份

- 同时备份到 1–4 个目的地，支持项目归档目录与镜像目录模式。
...
- 项目保存拍摄日期、机位、目标路径、卷标规则与各机位的下一卷编号。
...
应用内更新检查只访问项目官方 GitHub Release，不会静默下载或替换应用。外置卷推出前会检查备份与代理队列，防止移除仍在使用的设备。
...
- `Kocpy-0.0.3-arm64.dmg`：Apple Silicon Mac
- `Kocpy-0.0.3-x64.dmg`：Intel Mac
