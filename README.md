...
当前正式版：**0.1.37** · [下载](https://github.com/sexyfeifan/Kocpy/releases/latest) · [使用手册](docs/USER_GUIDE.md) · [文档导航](docs/README.md) · [本版更新](docs/RELEASE_NOTES_0.1.37.md)
...
下载后先核对同一 Release 的 [SHA256SUMS.txt](https://github.com/sexyfeifan/Kocpy/releases/download/v0.1.37/SHA256SUMS.txt)，再打开 DMG，将 Kocpy 拖入“应用程序”。
...
4. **自动报告与 NAS 归档**：每个已校验目的地默认保存 PDF，可关闭或单独重试；新增独立 NAS 转存、逐文件回读和 PDF／PNG 报告流程。

[完整更新说明](docs/RELEASE_NOTES_0.1.37.md) · [历史正式发布](https://github.com/sexyfeifan/Kocpy/releases)

### 验证范围与已知限制

0.1.37 的精确自动测试、架构、候选包与正式附件验收范围见[验证记录](docs/VERIFICATION.md)和同版 Release；任何未执行项目不会计作通过。

- 没有 Developer ID／Apple 公证，不宣称通过 App Store 审核。
- 历史硬件或双机结果不能泛化成当前版本在所有设备上通过；真实外置双盘、NAS 网络中断／重挂载、拔盘、睡眠与空间耗尽的本版复验边界仍明确保留。
...
Separate arm64 and x64 installers are available. They currently use ad-hoc signing, **not Developer ID signing or Apple notarization**. Automated tests and historical hardware results are not certification for every storage setup; this release does not claim complete real-NAS or two-independent-external-disk coverage.
...
Apple Silicon／Intel 用インストーラは別々です。現在は ad-hoc 署名のみで、**Developer ID 署名・Apple 公証はありません**。自動テストをあらゆる実機構成の検証とみなさないでください。すべての実 NAS／独立した外付け二台構成を検証済みとは表明していません。
