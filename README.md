...
项目目录采用固定层级：`项目开始日期_项目名 / 当天拍摄日期 / 设备或机位 / 素材卷`。例如 `20260827_山海之间/20260829/FX3/FX3_003/`。每个设备维护独立的素材卷编号，连续接卡时自动递增。
...
- 项目保存名称、拍摄周期、最多 10 个常用设备或机位、各设备素材卷前缀、目标根目录与独立递增卷号。
...
应用启动后自动检查项目官方 GitHub Release；发现新版本时按当前 Mac 架构匹配 Apple Silicon 或 Intel DMG，并提供直接升级包按钮和版本说明入口。应用不会静默下载或替换自身。外置卷推出前会检查备份与代理队列，防止移除仍在使用的设备。
...
- `Kocpy-0.0.4-arm64.dmg`：Apple Silicon Mac
- `Kocpy-0.0.4-x64.dmg`：Intel Mac
