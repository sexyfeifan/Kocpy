...
当前代码版本：**0.1.30** · [完整使用手册](docs/USER_GUIDE.md) · [0.1.30 更新说明](docs/RELEASE_NOTES_0.1.30.md) · [已公开发布版本](https://github.com/sexyfeifan/Kocpy/releases/latest)

0.1.30 将历史卡识别与项目完成动作收口为可解释、需授权的安全自动化：目录结构匹配只提示疑似重复，卷历史建议列出依据并只填写草稿；任务完成只建立待办建议，用户填写操作人并逐项确认后才生成报告、交付清单、代理或推出源盘。动作有稳定幂等键、失败记录和非覆盖发布，不修改 MHL 或备份校验结论。已公开安装包以 Release 实际附件为准，草稿不是公开发行；验证边界及已知限制见 [验证记录](docs/VERIFICATION.md)。
...
- `Kocpy-0.1.30-arm64.dmg`：Apple Silicon Mac
- `Kocpy-0.1.30-x64.dmg`：Intel Mac
